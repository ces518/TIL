...
```java
@ExceptionHandler
public String eventExceptionHandler (EventException e, Model model) {
...
```java
@ExceptionHandler({Exception.class, EventException.class})
public String eventExceptionHandler (Exception e, Model model) {
	model.addAttribute("message", e.getMessage());
	return "error";
}
```
