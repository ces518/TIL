...
### AMQP 의 RPC 프레임 구조
- AMQP 스펙은 프로그래밍 언어의 객체지향 개념과 유사한 클래스와 메소드를 사용하면서 클라이언트/서버 간의 공통 모델이 정의되어 있다.
  - AMQP 클래스 : 기능의 범위를 정의
  - AMQP 메소드 : 각 클래스에 서로 다른 작업을 수행
...
- RabbitMQ 에서 AMQP 명령을 전송/수신할 때 필요한 모든 인자들은 데이터 구조로 캡슐화된 **프레임** 으로 인코딩 되어 전송됨
  - 프레임 => 명령과 해당 인지를 인코딩해 각 프레임이 서로 구분되도록 하는 효율적인 방법
...
- 콘텐츠 헤더 프레임과 바디 프레임이 그 다음으로 전송된다
  - 콘텐츠 헤더는 본문 크기와 메세지 속성이 포함되어 있고, 
  - 메세지 본문이 AMQP 에 정의된 최대 프레임크기를 초과하면 **여러 바디 프레임 으로 분할** 전송 된다
- 메세지 전송시 하나의 메세지 덩어리는 **항상 메소드 > 콘텐츠 헤더 > 바디 프레임 순으로 전송된다**
...
- 메소드 프레임은 RPC 요청이 처리할 클래스/메소드 와 실행을 위한 인자를 함께 전달한다
...
### 익스체인지 선언
- AMQP 모델에서 익스체인지는 큐와 같이 1급 시민으로 취급된다
...
- 요청 받은 익스체인지를 발견하면 라우팅 키와 일치하는 큐를 찾는다
