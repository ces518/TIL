...
- 하나의 스레드가 아닌 여러 스레드로 처리할 수 있게 **멀티 스레드 복제 기능** 을 제공한다.
- 5.6 미만에서는 레플리카 서버는 무조건 하나의 스레드로만 복제를 수행했음.
- 멀티 스레드 복제 기능은 5.6 버전에서 처음 도입 되었다.

![MySQL Multi Thread Replication](./images/mysql_multi_thread_replication.png)
- 기존 단일 스레드 복제에서는 레플리케이션 SQL 스레드가 릴레이 로그를 읽어 바로 적용했다면 멀티 스레드 복제에서 SQL 스레드는 **코디네이터 스레드 (Coordinator Thread)** 로 불린다.
- 실제로 이벤트를 실행하는 스레드인 **워커 스레드 (Worker Thread)** 와 협업해 동기화를 진행한다.
- 멀티 스레드 복제는 소스 서버로부터 복제된 트랜잭션 들을 **어떻게 병렬 처리할 것인가** 에 따라 방식이 나뉜다.
  - 데이터베이스 기반, LOGICAL CLOCK 기반
- `slave_parallel_type` 시스템 변수로 설정이 가능하며 `slave_parallel_workers` 시스템 변수로 워커 스레드 개수를 지정할 수 있다.
  - 기본적으로 데이터베이스 기반으로 설정되며, 워커 스레드는 0 ~ 1024 까지 설정이 가능하다.
  - 워커 스레드를 0으로 설정하면 기존 단일 스레드 복제 모드로 수행하게 됨.
- `slave_pending_jobs_size_max` 시스템 변수를 통해 워커 스레드의 큐에 할당 할 수 있는 최대 메모리 크기를 설정할 수 있다.
  - 기본 값은 8.0 기준 128MB 이고 OLTP 환경에서는 기본값 그대로 사용해도 무방하다.

`데이터베이스 기반 멀티 스레드 복제`
- 데이터베이스 기반 멀티 스레드 복제 방식은 **스키마 기반 (Schema-based)** 처리 방식이라고도 한다.
- MySQL 내의 데이터베이스 단위로 병렬 처리를 수행하는 형태
- 데이터베이스가 하나뿐이라면 아무런 장점을 가지지 못함.
- 여러 개의 데이터베이스가 있다면 레플리카 서버에서 간으하면 해당 개수만큼 워커 스레드 수를 설정하는 것이 좋음.

![MySQL Database based Replication](./images/mysql_database_based_replication.png)

- 데이터베이스 기반 멀티스레드 복제에서는 테이블/레코드 수준의 충돌 여부는 고려하지 않고, 데이터베이스가 동일한지 아닌지만 비교해 병렬 처리를 수행한다.
- 서로 다른 데이터베이스를 참조하는 쿼리나 트랜잭션이 빈번하게 실행되는 경우 예상보다 처리 ㅎ율이 낮아질 수 있다.
- 멀티 스레드 복제에서는 레플리카 서버거 자체적인 바이너리 로그를 가지고 있고, `log_slave_updates` 옵션이 활성화 되어 있는 경우 소스 서버의 바이너리 로그에 기록된 트랜잭션 순서와 레플리카 서버의 바이너리 로그에 기록된 트랜잭션 순서가 다를 수 있음.
- 각 트랜잭션이 처리된 시점에 따라 실제 소스 서버에서 실행된 것과는 순서가 달라질 수 있기 때문이다.

`LOGICAL CLOCK 기반 멀티스레드 복제`
- 5.7 버전 부터 데이터베이스에 종속되지 않고 멀티스레드로 처리하는, 같은 데이터베이스 내에서도 멀티 스레드 동기화 처리가 가능한 `LOGICAL CLOCK` 방식이 동비되었다.
- 이는 소스 서버에서 트랜잭션들이 바이너리 로그로 기록될 때 **각 트랜잭션 별 논리적인 순번을 부여** 해 해당 순번을 기반으로 정해진 기준에 따라 병렬로 실행할 수 있게 하는 방식.
- 이 또한 `LOGICAL CLOCK` 의 세부 처리 방식에 따라 달라질 수 있는데, 크게 세가지로 나뉜다.
  - Commit-parent/Lock/Write Set 

`바이너리 로그 그룹 커밋`

![MySQL Binlog Group Commit](./images/mysql_binary_log_group_commit.png)

- MySQL 5.5 버전까지는 InnoDB 스토리지 엔진에서 한 시점에 하나의 트랜잭션만 커밋할 수 있었으며 바이넌리 로그에 트랜잭션을 기록하고 디스크에 동기화 하는 부분도 동일 했다.
- 클라이언트로 부터 커밋 요청이 들어오면 `Prepare` 와 `Commit` 두 단계를 거쳐 커밋을 처리하는데 이를 **분산 트랜잭션 (XA, Two-Phase Commit)** 이라고 한다.
- 이는 트랜잭션 커밋시 스토리지 엔진에 적용된 내용과 바이너리 로그에 기록된 내용의 일관성을 유지하기 위해 사용함.
- 커밋 처리 과정중 바이너리 로그에 기록한 내용을 동기화하는 `fsync` 작업은 `sync_binlog` 시스템 변수에 설정된 값에 따라 실행 빈도가 결정된다.
  - 1인 경우 매 트랜잭션 커밋 마다 동기화가 수행되며 서버에 부하를 야비했다.
  - 5.6 버전에서는 여러 트랜잭션에 대한 커밋을 동시에 처리가능하도록 변경되었음.
- 이를 **바이너리 로그 그룹 커밋** 기능 이라고 한다.
- 트랜잭션들은 커밋 처리 과정중 `Prepare` 이후 바이너리 로그 관련 처리 진행시 다음과 같이 세단계를 거치며 최종적으로 그룹 커밋 된다.

![MySQL Binlog Group Commit](./images/mysql_binlog_group_commit.png)
- 각 단계에 큐가 존재하고, 트랜잭션은 순서대로 큐에 등록된다.
- 비어있는 큐에 첫 번째로 등록된 트랜잭션을 **리더 (Leader)** 라고 함.
- 이후 등록된 트랜잭션들은 모두 **팔로워 (Follower)** 
- 팔로워는 리더에게 자신의 트랜잭션 처리를 모드 일임하고, 리더는 큐에 등록된 모든 팔로워들을 가져와 처리하고 다음 단계의 큐에 등록한다.
- 리더는 팔로워가 될 수 있지만, 팔로워는 절대 리더가 될 수 없다.

각 단계에서 트랜잭션들은 다음과 같이 처리된다.
1. Flush 단계
   - 대기 큐에 등록된 트랜잭션들을 순서대로 바이너리 로그에 기록
2. Sync 단계
   - 기록된 바이너리 로그를 디스크와 동기화하는 fsync() 시스템 콜 수행
   - `sync_binlog` 옵션 설정에 따라 디스크 동기화를 진행함
   - 1인 경우 매번 디스크 동기화 작업 수행
3. Commit 단계
   - 대기 큐에 등록된 트랜잭션에 대해 스토리지 엔진 커밋 진행

> Sync 단계에서 더 많은 트랜잭션을 쌓아 한번에 처리 될 수 있도록 시스템 변수를 통해 Sync 단계 실행을 지연시킬 수 있음.

`binlog_group_commit_sync_delay`
- 디스크 동기화 작업을 얼마나 지연시킬지 제어하는 변수이며 단위는 마이크로 초 이다.
- 기본값 0
- 0보다 큰 값으로 설정하면 fsync() 시스템 콜 호출 수를 줄일 수 있음.
- 값이 크게 설ㅈ어될 수록 트랜잭션 커밋 완료까지 시간이 오래걸리기 때문에 응답이 지연될수 있다.
- 적절한 값으로 설정하는 것이 중요함.

`binlog_group_commit_sync_no_delay_count`
- 바이너리 로그 동기화 작업 진행되기 전 지연되어 대기할 수 있는 최대 트랜잭션 수를 설정하는 변수
- 위에서 설정한 지연시간이 남아 있더라도 이 변수에 지정한 수 만큼 대기 트랜잭션이 차게 되면 디스크에 동기화 하게 된다.
- 위 설정값이 0이라면 무시됨

Commit 단계에서 대기 큐에 등록된 트랜잭션들에 대해 스토리지 엔진 커밋을 진행하고, 커밋은 큐의 순서대로 또는 병렬로도 처리될 수 있다.
- 병렬로 처리되는 경우 다음 시스템 변수들을 통해 트랜잭션 커밋 순서 제어가 가능하다.

`binlog_order_commits`
- 불리언 변수로 0/1 이나 ON/OFF 로 설정 가능하다.
- 기본값은 1 (ON)
- 활성화 된 경우 트랜잭션 들이 바이너리 로그 파일에 기록된 순서대로 커밋됨
- 0이라면 순서와 관계없이 병렬처리된다.

`Commit-parent 기반 LOGICAL CLOCK 방식`
- Commit-parent 기반 LOGICAL CLOCK 방식은 멀티스레드 복제 동기화 최초 도입되었던 5.7.2 ~ 5.7.5 까지 적용된 방식
- 동일 시점에 커밋된 트랜잭션 들을 레플리카 서버에서 병렬로 실행할 수 있게 한다.
  - 커밋시점이 동일한 경우 락 경합과 같이 충돌한느 부분이 없으므로 병렬 실행가능하다는 점에서 착안되었다.
- MySQL 서버는 같은 시점에 커밋된 트랜잭션을 식별하기 위해 바이너리 로그 기록시 `commit_seq_no` 라는 값을 함께 기록한다.
  - 해당 트랜잯녀 커밋시 갖아 최근에 커밋된 트랜잭션 순번 값이 저장됨.
  - MySQL 내부에서 글로벌 하게 관리되는 commit clock 이라는 64 비트 정수값 기반
- `commit_seq_no` 는 트랜잭션 커밋을 위한 `Prepare` 단계 진입시 설정된다.
  - 트랜잭션이 최종적으로 스토리지 엔진에 커밋되기 전 증가됨.
- 같은 시점에 커밋 처리가 시작된 트랜잭션은 동일한 commit_seq_no 값을 갖게 되고 이를 기반으로 병렬 처리를 하게 된다.
  - 병렬 처리 기준 (commit_seq_no) 이 앞서 커밋된 트랜잭션 커밋 순번값이기 때문에 이 방식을 Commit-parent 방식이라고 하는 것.
- 이 방식은 소스 서버에서 **같은 그룹 으로 커밋된 트랜잭션 수가 많으면 많을수록** 레플리카 서버에서의 트랜잭션 병렬 처리 율이 향상된다.

`잠금 기반 LOGICAL CLOCK 방식`
- 5.7.6 버전부터는 잠금 기반  LOGICAL CLOCK 이 도입되어 Commit-Parent 기반 LOGICAL CLOCK 은 더이상 사용되지 않는다.
- 잠금 기반 방식에서는 선행 트랜잭션 순번 값이 동일하지 않아도 커밋 처리 시점이 겹친다면 병렬로 처리가 가능하다.
- 이를 위해 바이너리 로그에 `sequence_number` 와 `last_committed` 값을 함께 기록한다.
  - `sequence_number` 는 커밋된 트랜잭션에 대한 논리적인 순번값. 매 커밋마다 증가한다.
  - `last_committed` 는 현 트랜잭션 이전에 커밋된 가장 최신 트랜잭션의 시퀀스 값이 저장됨

`WriteSet 기반 LOGICAL CLOCK 방식`
- MySQL 8.0.1 버전에 도입된 방식.
- 트랜잭션 커밋 처리 시점이 아닌 **트랜잭션이 변경한 데이터를 기준** 으로 병렬 처리 가능 여부를 결정함.
  - 기존 잠금 방식에서는 커밋 처리 시점이 전혀 겹치지 않는 트랜잭션을 병렬 실행이 불가능 했음.
- 동일한 데이터를 변경하지 않는 트랜잭션 들은 레플리카 에서 모두 병렬로 실행 가능하다.
- WriteSet 기반에서는 같은 세션에서 실행된 트랜잭션들의 병렬 처리 여부에 따라 `WRITESET` 과 `WRITESET_SESSION` 타입으로 다시 나뉜다.
- `binlog_transaction_dependency_tracking` 시스템 변수를 통해 원하는 타입 설정이 가능하다.
  - `COMMIT_ORDER`
    - 기본값.
    - 5.7 버전의 잠금 기반과 동일하게 동작
  - `WRITESET`
    - 서로 다른 데이터를 변경한 트랜잭션 들은 모두 병렬 처리가 가능.
  - `WRITESET_SESSION`
    - 동일 세션에서 실행된 트랜잭션 들은 병렬 처리될 수 없다는 것 외에 WRITESET 과 동일하다.
- 각 트랜잭션에서 변경한 데이터를 기준으로 병럴 처리를 위해 트랜잭션들의 **종속 관계** 를 정의한다.
  - 이를 위해 트랜잭션에 의해 변경된 데이터 들의 목록을 관리함.
- 이는 하나하나가 모두 해시값으로 표현되는데 이 해싱된 데이터를 `WriteSet` 이라고 한다.

```shell
// WriteSet 이 생성되는 조합
WriteSet = hash(index_name, db_name, db_name_length, table_name, table_name_length, value, value_length)
```
- WriteSet 은 테이블에 존재하는 유니크 키의 개수 만큼 만들어지기 때문에 하나의 변경 데이터는 여러 WriteSet 을 가질 수 있다.
- WriteSet 생성시 사용하는 해시 알고리즘은 `transaction_write_set_extraction` 시스템 변수에 지정된 알고리즘이 사용된다.
  - OFF, MURMUR32, XXHASH64
  - 기본값은 XXHASH64
- `binglog_transaction_dependency_tracking` 시스템 변수가 `WRITESET` 또는 `WRITESET_SESSION` 인 경우 `transaction_write_set_extraction` 시스템 변수를 OFF 로 지정할 수 없다.
  - 다른 값으로 변경하려면 바이너리 로그 포맷이 `ROW` 여야 함.
- WriteSet 은 MySQL 서버 메모리에서 **해시맵 (HashMap) 테이블** 로 히스토리가 관리된다.
  - 변경된 데이터의 해시값과 해당 데이터를 변경한 트랜잭션의 `sequence_number` 값이 Key-Value 형태로 저장됨.
- `binlog_transaction_dependency_history_size` 시스템변수로 최대 해싱 데이터 개수를 설정할 수 있다.
  - 기본값 25,000
  - 저장된 데이터 수가 최대 값에 도달하면 히스토리 테이블은 초기화디고, DDL 이 실행된 경우에도 초기화 된다
- WriteSet 기반 방식에서도 트랜잭션 커밋시 바이너리 로그에 `last_committed`, `sequence_number` 값이 기록된다.
  - 트랜잭션의 `last_comitted` 값을 1차적으로 COMMIT_ORDER 타입 기반으로 설정하고 이후 히스토리 테이블의 데이터를 이용해 충돌 여부를 확인 후 다시 last_committed 값을 설정함.

![MySQL WriteSet LOGICAL CLOCK](./images/mysql_writeset_logical_clock.png)

- 트랜잭션에서는 만약 WriteSet 히스토리 테이블에 자신의 WriteSet 과 충돌하는 데이터가 존재하는 경우 해당 WriteSet 에 매핑된 sequence_number 를 가져와 자신의 last_committed 에 저장한 후 sequence_number 를 자신의 sequence_number 로 변경한다.
  - 만약 충돌하는 WriteSet 이 여러개 라면 last_committed 에는 sequence_number 가 가장 큰 값이 저장됨.
  - 충돌이 발생하지 않는 다면 해당 트랜잭션이 가진 WriteSet 이 히스토리 테이블에 새로 저장되고, last_committed 에는 히스토리 테이블에 존재하는 가장 작은 sequence_number 가 저장됨.
- `WRITESET_SESSION` 타입에서는 last_committed 값이 결정된 후 같은 세션에서 커밋된 마지막 트랜잭션의 sequence_number 와 비교해 둘중 더 큰값을 선택해 최종적으로 last_committed 에 저장하게 된다.
- WRITESET, WRITESET_SESSION 모두 유니크 키가 없다면 해당 트랜잭션의 WriteSet 은 생성되지 않고 COMMIT_ORDER 기반으로 결정된 값을 사용하게 된다.
  - 유니크 키들이 다른 테이블에서 FK 로 참조되는 경우에도 동일
- WriteSet 방식은 바이너리 로그 포맷이 `ROW` 여야만 정상적으로 동작하게 된다.

> `WRITESET` 타입을 사용하면 체인 복제 구조에서의 병렬 처리량 감소 문제를 해결할 수 있지만, 추가적인 메모리 공간 사용과 오버헤드가 발생한다.

#### 멀티 스레드 복제와 복제 포지션 정보
- 멀티 스레드 복제에서 각 워커 스레드 들이 실행한 바이너리 로그 이벤트의 포지션 정보는 `relay_log_info_repository` 시스템 변수에 저장된 값에 의해 설정된다.
  - `mysql.slave_worker_info` 테이블 혹은 데이터 디렉터리 내에 `worker_relay_log.info` 파일에 스레드별로 저장됨
- 각 워커 스레드 들은 이벤트 실행 완료마다 해당 데이터를 갱신함.
- 현재 복제 이벤트의 처리 현황을 보여주는 어플라이어 메타데이터는 워커 스레드들이 실행한 이벤트에서 로우 워터 마크에 해당하는 이벤트 포지션 값이 저장된다.
  - 이는 코디네이터 스레드 (SQL 스레드) 가 수행하는 체크포인트 작업에 의해 주기적으로 갱신.
- 코디네이터 스레드는 다음 시스템변수들에 설정된 값을 바탕으로 워커 스레드들에서 실행된 이븐트들에 대한 체크포인트를 수행한다.
  - `slave_checkpoint_period`
    - 코디네이터 스레드의 어플라이어 메타데이터 갱신 작업 의 실행 주기
    - 기본 값은 300 이며 단위는 밀리초
  - `slave_checkpoint_group`
    - 위와 동일한 역할로 사용됨.
    - 트랜잭션의 개수를 지정한다.
    - 10으로 지정한다면 10개 트랜잭션 수행 후 체크포인트 작업 수행.

### 크래시 세이프 복제 (Crash-safe Replication)
- MySQL 서버 운영중 예기치 못한 장애로 비정상 되는 경우가 종종 있다.
- 레플리카 서버였다면 비정상 종료후 소스서버와 복제 동기화가 실패할 수 있음 
  - 특히 릴레이 로그의 디스크 동기화 설정이 안된 경우
- 비정상 종료후 재구동 됬을때 복제가 원활하게 진행될 수 있더록 여러 설정들을 제공한다.
- 이를 통해 서버 장애 이후 문제 없이 복제가 진행 되는 **크래시 세이프 복제** 가 가능해진다.

`서버 장애와 복제 실패`
- 레플리케이션의 I/O 스레드와 SQL 스레드가 협업해 소스 서버와의 데이터 동기화를 수행한다.
- 이 과정에서 I/O 스레드와 SQL 스레드는 각각 자신이 어느 시점까지 바이너리 로그를 가져왔는지 어느 트랜잭션까지 재실행했는지에 대한 **포지션 정보** 를 남겨 두었다가 종료후 재구동시 해당 포지션 정보를 참조해 복제를 시작한다.
- 포지션 정보는 `master_info_repository`, `relay_log_info_repository` 시스템 변수에 설정된 값에 따라 파일이나 테이블로 관리 된다.
  - 파일로 관리할 경우 원자적으로 동기화 상태를 관리할 수 없다.
  - 비정상 종료시 처리 내역과 포지션 정보간에 불일치가 발생하는 문제가 있음.
- 5.6 버전부터 테이블로 관리하는 기능이 제공되며 덕분에 원자적으로 관리가 가능해짐.

![MySQL Replication Sync](./images/mysql_replication_sync.png)

`크래시 세이프 복제를 위한 최소한의 적용 옵션`

```shell
replay_log_recovery=ON
replay_log_info_repository=TABLE
```
- 유의할 것은 서버의 비정상종료시에만 크래시세이프할뿐, 서버 장비나 운영체제가 비정상 종료된다면 무용지물일 수 있다.

`복제 사용 형태별 크래시 세이프 복제 설정`
- 복제 타입및 동기화 방식에 따라 크래시 세이프 복제 설정이 조금씩 달라진다.
- 크게 4가지로 나눠 설정을 살펴봄.

`바이너리 로그 파일 위치기반 복제 + 싱글 스레드 동기화`
...
`바이너리 로그 파일 위치기반 + 멀티 스레드 동기화`
- 레플리카 서버에서 복제된 트랜잭션들의 커밋 순서가 소스 서버와 동일하도록 설정되었는지에 따라 옵션 셋이 다라진다.
...
```
- sync_relay_log 옵션의 기본값은 10000 이고, 1이 아닌 0이나 1 이상값을 사용하면 비정상 종료시 로그 내용이 유실될 수 있다.
  - 멀티 스레드 동기화시 발생할 수 잇는 **트랜잭션 갭** 때문
- **트랜잭션 갭 (Transaction Gap)** 은 멀티 스레드 복제에서 병렬 처리로 인해 트랜잭션들이 순서대로 처리되지 않아 일시적으로 발생하는 트랜잭션 들의 간격을 의미.
- `relay_log_recovery` 옵션 활성화시 MySQL 재구동시 트랜잭션 갭을 메우는 작업이 이뤄짐.
- sync_relay_log 이 1일 경우 매번 디스크 동기화되어 유실가능성이 적지만 그만큼 부하가 발생하고 복제 성능에도 영향을 준다.
- 때문에 멀티스레드 동기화시 LOGICAL_CLOCK 방식과 slave_preserve_commit_order=1 를 주어 사용하는 것을 권장.

`GTID 기반 복제 + 싱글 스레드 동기화`
- GTID 기반 + 싱글 스레드 복제 동기화시 `mysql.gtid_executed` 테이블 데이터가 복제된 트랜잭션이 적용될 때마다 매번 함께 갱신되는지 여부에 따라 크래시 세이프 복제를 위한 옵션 셋이 달라짐.

```shell
# gtid_executed 테이블 데이터가 매 트랜잭션 적용마다 갱신되는 경우
...
- GTID 기반 + 멀티스레드 동기화시 싱글 스레드 동기화와 동일하다.
- 유의할 점은 비정상적 적용후 `relay_log_recovery=ON` 설정으로 재구동시 트랜잭션 갭을 매우는 작업이 수행됨.
- 하지만 이는 SOURCE_AUTO_POSITION 옵션을 사용하는 GTID 기반 복제에서는 불필요한 작업이다.
- 이런 작업은 해당 옵션 사용 유무와 관계없이 실행되서 릴레이 로그가 손실되는 경우 복주 작업시 갭 매우기에 실패하며 복제 실패가 되는 경우가 있었음.
- 8.0.18/5.7.28 버전 부터는 이런 작업이 생략되도록 수정되었다.
- 위 버전 미만일 경우 아래 와 방식으로 처리함으로 복제를 정상 처리가 가능하다.
...
- MySQL 복제에서는 소스 서버의 특정 이벤트들만 레플리카 서버에 적용될 수 잇도록 필터링 기능을 제공한다.
- 필터링의 주체는 소스/레플리카 서버 둘다 가능.
- 레플리카 서버에서 좀더 다양한 필터링이 가능하다.
- 두가지 옵션을 통해 설정 가능
  - binlog_do_db
    - 바이너리 로그에 기록할 DB 명 지정.
    - 해당 DB 들의 이벤트만 바이너리 로그에 기록됨.
  - binlog_ignore_db
    - 위와 상반되는 옵션
  - 두 옵션을 설정 파일에도 지정해 사용할 수 있다.
...
```
- 레플리카 서버의 필터링은 릴레이 로그에 저장된 **이벤트를 실행할 때 적용하는 것.**
- 서버가 구동중인 상태에서 `CHANGE REPLICATION FILTER` 구문을 통해 복제 필터링 설정이 가능하다.

```shell
CHANGE REPLICATION FILTER filter[, filter, ...] [FOR CHANNEL channel_name]

filter:
  REPLICATION_DO_DB = (db_name[, db_name, ...])
  ....
```

`필터링 옵션 목록`

| 옵션 | 커맨드라인 및 설정 파일 옵션 | 설명 |
| --- | --- | --- |
| REPLICATE_DO_DB | replicate_do-db | 복제 대상 DB 지정 |
| REPLICATE_IGNORE_DB | replicate-ignore-db | 복제 제외 DB 지정 |
| REPLICATE_DO_TABLE | replicate-do-table | 복제 대상 테이블 지정 |
| REPLICATE_IGNORE_TABLE | replicate-ignore-table | 복제 제외 테이블 지정 |
| REPLICATE_WILD_DO_TABLE | replicate_wild-do-table | 복제 대상 테이블을 와일드 카드 패턴으로 지정 |
| REPLICATE_WILD_IGNORE_TABLE | replicate_wild-ignore-table | 복제 제외 테이블을 와일드 카드 패턴으로 지정 |
| REPLICATE_REWRITE_DB | replicate-rewrite-db | 특정 DB 에 대한 이벤트를 지정한 DB 로 치환해 적용 |

> CHANGE REPLICATION FILTER 구문으로 필터링 적용시 복제 시작전 실행하거나 복제를 멈추었다가 실행후 재시작하면 필터링이 적용 된다.
